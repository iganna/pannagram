...
file.nestedness = "~/Sources/pannagram_test/test_ref_free_acc/features/sv/simsearch_sv_80_80/seq_sv_large_80_80.txt"
file.nestedness <-"~/Sources/pannagram_test/test_ref_free_acc/features/sv/tmp/seq_sv_large_85_85.txt"
```
...
cov.cutoff = 80
edges = getGraphFromNestedness(nestedness, cov.cutoff = cov.cutoff)
...
```{r}
seqs = readFasta("~/Sources/pannagram_test/test_ref_free_acc/features/sv/tmp/seq_sv_large.fasta")


edges.major.no.forks <- filterEdgesForks(edges = edges.no.shortcut,
                                         seqs = seqs)
...
# Filter Umbrela Edges
```{r}


edges.major.no.umbrella <- filterEdgesUmbrella(edges = edges.major.no.forks,
...
g %v% "size" <- graph.compact$nodes.size[g.names]
set.seed(seed.value)
p.compact <- ggnet2(g, label = F, edge.color = "black",
            node.size = 'size',
            color = '#468B97',
            arrow.gap = 0.01, arrow.size = 2,
) + theme(legend.position = "none")
...
partition = getGraphCommunities(edges.major.no.umbrella) 
...
g.label[names(partition)] = as.character(partition)
...
partition.solved = getGraphComponents(edges.solved) 
partition.solved = partition.solved$membership
...
seqs.names.comp = names(partition.solved)[partition.solved == i.comp]
...
dotplot(seqs.comp[1], seqs.comp[1])
...
path.mafft = "~/Sources/pannagram_test/test_ref_free_acc/features/sv/tmp/mafft/"
