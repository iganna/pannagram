...
# Remain those nodes, that have at least three sequences + Length cutoff
```{r}
...
nrow(edges.solved)
edges.solved.filter <- solveForkNodes(edges = edges.solved,
                                         seqs = seqs)
...
edges.fam <- filterEdgesForks(edges.fam,seqs = seqs)

edges.fam.copact <- getGraphCompact(edges.fam)

g <- network(edges.fam.copact$edges, matrix.type = "edgelist", ignore.eval = FALSE, directed = TRUE)
g.names = network.vertex.names(g)
g %v% "size" <- edges.fam.copact$nodes.size[g.names]
set.seed(seed.value)
p <- ggnet2(g, label = F, edge.color = "black",
            node.size = 'size',
            color = '#468B97',
            arrow.gap = 0.05, arrow.size = 3,
)
p
...
# Plot Families
```{r}
path.fam = "~/Sources/pannagram_test/families/"
...
path.mafft = "~/Sources/pannagram_test/families/tmp/"
