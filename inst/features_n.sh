...
# features_n -- the same as `features`, but it reads alignments stored in the
# INTERVAL format produced by `pannagram_n` (see utils/interval_func.R).
# Only the scripts that touch the /accs payload have `_n` counterparts.
...
if [ "$run_blocks" = true ]; then # -blocks
    ...
    Rscript $INSTALLED_PATH/analys/analys_01_blocks3_n.R \
...
if [ "$run_seq" = true ]; then # -seq
    ...
    mkdir -p $path_seq
    Rscript $INSTALLED_PATH/analys/analys_02_seq_cons_n.R \
        --path.features.msa ${path_features_msa} \
        --ref  ${ref_pref} \
        --path.chr ${path_chrom} \
        --path.seq ${path_seq} \
...
if [ "$run_snp" = true ]; then # -snp
    ...
    Rscript "$INSTALLED_PATH/analys/analys_04_snp_2_n.R" \
        --path.features.msa "${path_features_msa}" \
        --path.snp "${path_snp}" \
        --path.seq "${path_seq}" \
        --ref "${ref_pref}" \
...
if [ "$run_sv_call" = true ]; then # -sv_call|-sv
    ...
    Rscript $INSTALLED_PATH/analys/sv_01_calling_n.R \
        --path.features.msa ${path_features_msa} \
        --path.seq ${path_seq} \
        --path.sv ${path_sv} \
        --path.gff ${path_gff} \
