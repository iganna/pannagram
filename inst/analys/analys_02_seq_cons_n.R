...
option_list = list(
  ...
  make_option("--chunk.seq",         type = "integer",  default = 65536, help = "Chunk size of the /accs datasets in seq_*.h5 (0 = one chunk for the whole vector, the old behaviour)"),
...
# Chunking of seq_*.h5. h5write() on its own makes ONE chunk for the whole vector, so
# any windowed read has to inflate the entire accession: 20 windows of 301 columns
# cost 0.57 s that way against 0.09 s at 64K, for ~2% more on disk. The alignment
# itself no longer lives here (see utils/interval_func.R), but the nucleotides do.
chunk.seq <- if(is.null(opt$chunk.seq)) 65536L else opt$chunk.seq
...
# # for(s.comb in pref.combinations){
  ...
  for(acc in accessions){
    ...
    if(acc %in% acc.exist){
      ...
      suppressMessages({
        try(h5delete(file.seq, s.acc), silent = TRUE)   # idempotent on resume
        if(chunk.seq > 0){
          h5createDataset(file.seq, s.acc, dims = length(s),
                          storage.mode = "character", size = 1,
                          chunk = min(chunk.seq, length(s)),
                          level = 6, filter = "gzip", shuffle = TRUE)
        }
        h5write(s, file.seq, s.acc)
      })
    }
