...
aln.type.out = paste0(aln.type.out, '_')
...
for(s.comb in pref.combinations){
  ...
  # No /blocks group any more (the block id is a column of every interval table)
  ...
  len.comb = h5AccLen(file.comb, accessions[1])
  ...
  for(acc in accessions){
    ...
    v = h5VecRead(file.comb, acc, len.comb)
    v[is.na(v)] = 0
    ...
    pokaz('Save new...')
    h5VecWrite(file.out, acc, v.new)
    markDone(acc.id, file=file.log.loop, echo=echo.loop)

  }

  h5PanLenSet(file.out, len.new)
