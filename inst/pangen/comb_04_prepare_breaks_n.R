...
for(s.comb in pref.combinations){
  ...
  parallel::clusterEvalQ(cl, source(system.file("utils/interval_func.R", package = "pannagram")))
  ...
  res <- foreach(
    ...
    # One interval table replaces both the /accs vector and the /blocks vector:
    # `ivlBlockAt` answers the former `b.acc[abs(...)]` lookup by binary search.
    ivl.acc <- h5IvlRead(file.comb, acc)
    x.acc   <- ivlToVec(ivl.acc, h5AccLen(file.comb, acc))
    ...
    if (any(idx.no.zero)) {
      idx.no.zero[idx.no.zero] <-
        ivlBlockAt(ivl.acc, abs(x.beg[idx.no.zero])) ==
        ivlBlockAt(ivl.acc, abs(x.end[idx.no.zero]))
    }
