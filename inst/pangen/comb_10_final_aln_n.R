...
for(s.comb in pref.combinations){
  ...
  len.aln.synteny = h5AccLen(file.comb, accessions[1])
  ...
  # Interval tables instead of full vectors: the peak memory of this loop was
  # n.acc * len.aln.new * 8 bytes (tens of GB on large pangenomes). Only one full
  # vector is materialised at a time now; the round-trip is exact.
  ivl.list = vector('list', length(accessions)); names(ivl.list) = accessions
  for(acc in accessions){

    pokaz('Accession', acc)
    
    # Read accession alignment
    v = h5VecRead(file.comb, acc, len.aln.synteny)
    ...
    # Keep in memory for the single final write, as an interval table
    ivl.list[[acc]] = ivlFromVec(v.new)
    rmSafe(v.new)
  }
  ...
  for(acc in accessions){
    ...
    # Take from memory (no preliminary file) and drop the all-zero columns
    v.remain = ivlToVec(ivl.list[[acc]], len.aln.new)[idx.remain]
    ivl.list[[acc]] = NULL

    # Save as an interval table (idempotent)
    h5VecWrite(file.res, acc, v.remain)

  }

  h5PanLenSet(file.res, sum(idx.remain))
