...
for i in "${!accessions[@]}"; do
    ...
    count="${acc_counts[$i]}"
    for ((i_chr=1; i_chr<=count; i_chr++)); do
        echo "${path_consensus}seq_cons_${i_chr}_${i_chr}.fasta" >> "$temp_ref"
        echo "${path_parts}${acc_name}_chr${i_chr}.fasta" >> "$temp_acc"
        echo "${path_blast}${acc_name}_${i_chr}_${i_chr}_on_cons.txt" >> "$temp_out"
        echo "${path_log}${acc_name}_${i_chr}_${i_chr}_on_cons.txt" >> "$temp_log"
    done
done
