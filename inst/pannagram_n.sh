#!/bin/bash
#
# pannagram_n -- the same pipeline as `pannagram`, but the alignments are stored in
# the INTERVAL format: every /accs/<acc> dataset is an integer table
#     pan.beg | acc.beg | len | delta | block
# instead of a full vector along the pangenome coordinate (see
# utils/interval_func.R). Command line arguments are identical to `pannagram`.
#
# Only the steps that touch the HDF5 payload have `_n` counterparts; the rest of the
# pipeline is shared with `pannagram` and is called from the original scripts.
...
for ref0 in "${refs_all[@]}"; do  
    ...
    if [ "${step_num}" -ge "${step_start}" ] || [ ! -f ${step_file} ]; then
        ...
        # Run the step
        Rscript $INSTALLED_PATH/pangen/comb_01_one_ref_n.R \
...
for ((i = 1; i < ${#refs_all[@]}; i++)); do
    ...
    if [ "${step_num}" -ge "${step_start}" ] || [ ! -f ${step_file} ]; then
        ...
        Rscript $INSTALLED_PATH/pangen/comb_02_two_refs2_n.R \
...
if [ "${step_num}" -ge "${step_start}" ] || [ ! -f ${step_file} ]; then
    ...
    Rscript $INSTALLED_PATH/pangen/comb_03_cleanup_n.R \
            --path.features.msa ${path_features_msa} \
            --path.inter.msa ${path_inter_msa} \
            --accessions ${file_accessions} \
            --cores ${cores} \
...
if [ "${step_num}" -ge "${step_start}" ] || [ ! -f ${step_file} ]; then
    ...
    Rscript $INSTALLED_PATH/pangen/comb_04_prepare_breaks_n.R \
            --path.features.msa "${path_features_msa}" \
            --path.inter.msa "${path_inter_msa}" \
            --accessions ${file_accessions} \
            --cores "${cores}" \
...
if [ "${step_num}" -ge "${step_start}" ] || [ ! -f ${step_file} ]; then
    ...
    Rscript $INSTALLED_PATH/pangen/comb_10_final_aln_n.R  \
            --cores ${cores} \
            --path.features.msa "${path_features_msa}" \
            --path.inter.msa "${path_inter_msa}" \
            --path.inter.synteny "${path_inter_synteny}"\
...
if [ "${step_num}" -ge "${step_start}" ] || [ ! -f ${step_file} ]; then
    ...
    Rscript $INSTALLED_PATH/pangen/comb_11_extra_seqs2_n.R  \
            --cores ${cores} \
            --path.chromosomes "${path_chrom}" \
...
if [ "${step_num}" -ge "${step_start}" ] || [ ! -f ${step_file} ]; then
    ...
    Rscript $INSTALLED_PATH/pangen/comb_13_fill_aln_n.R  \
            --cores ${cores} \
            --path.extra ${path_extra_long} \
            --path.cons ${path_features_msa} \
            --path.log ${path_log_step} \
