...
# NAMESPACE is gitignored, so on a fresh clone it is absent. Rcpp::compileAttributes()
# (invoked by devtools::document() below) aborts unless a NAMESPACE file already exists.
# Seed a roxygen-owned stub so document() can (re)generate the real one.
if [ ! -f NAMESPACE ]; then
  printf '# Generated by roxygen2: do not edit by hand\n' > NAMESPACE
fi
