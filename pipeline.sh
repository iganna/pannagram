...
Rscript query_to_parts.R -n ${n_chr_query} -t fasta --path.chr  ${path_chr_acc} --path.parts ${path_parts} --part.len $part_len
...
# cores=1
# Rscript cmp_to_ref.R --path.work ${path_consensus}  --file.chr.len.ref ${path_chr_ref}chr_len.rds  --path.aln.pref ../${pref_global}_alignments_   \
